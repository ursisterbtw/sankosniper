...
async def display_transactions(transactions):
    ...
    for item in transactions:
        ...
        if is_buy and trader_username == streamer_username:
            if streamer_username not in streamer_follower_data:
        ...
        if streamer_username in LEGENDARY_SET and supply in LEGENDARY_BUY_PARAMS:
            ...
            if follower_count > 25000:
                buy_params = BULLSEYE_BUY_PARAMS.get(supply, None)
            elif follower_count > 1000:
                buy_params = TARGET_BUY_PARAMS.get(supply, None)
            elif follower_count > 500:
                buy_params = SMOL_BUY_PARAMS.get(supply, None)
